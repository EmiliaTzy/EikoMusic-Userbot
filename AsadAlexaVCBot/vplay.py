    ...
    if GRPPLAY or (m.from_user and m.from_user.is_contact) or m.outgoing:
        ...
        if replied:
            if replied.video or replied.document:
                huehue = await replied.reply("`Mengunduh`")
                ...
                if len(m.command) < 2:
                    ...
                    if pq == "720" or "480" or "360":
                        ...
                        Q = 720
                        await huehue.edit(
                            "`Hanya 720, 480, 360 Diperbolehkan` \n`Sekarang Streaming Di 720p`"
                        )
                try:
                    if replied.video:
                ...
                if len(m.command) < 2:
                    ...
                    hmmm = HighQualityVideo()
                    if search == 0:
                        await huehue.edit("`Tidak Menemukan Apa pun untuk Kueri yang Diberikan`")
                    else:
                        songname = search[0]
                        url = search[1]
...
async def vstream(client, m: Message):
    if GRPPLAY or (m.from_user and m.from_user.is_contact) or m.outgoing:
        chat_id = m.chat.id
        if len(m.command) < 2:
            await m.reply("`Berikan sebuah Link/LiveLink/.m3u8 URL/YTLink untuk stream dari 🎶`")
        else:
            if len(m.command) == 2:
                link = m.text.split(None, 1)[1]
                Q = 720
                huehue = await m.reply("`Mencoba Untuk Stream 💭`")
            ...
            if hm == 0:
                ...
                if chat_id in QUEUE:
                    ...
                    try:
                        ...
                        await huehue.edit(
                            f"Memulai **[Live Stream 📺]({link})** di `{chat_id}`",
                            disable_web_page_preview=True,
                        )
                    except Exception as ep:
                        await huehue.edit(f"`{ep}`")
